...
## D-015: Additive Profile Tenant Ownership Foundation

*   **Status:** APPROVED AND IMPLEMENTED
*   **Context:** JWT `orgId` identifies `organizations.id`, but producer and certifier ownership relied on the temporary convention that profile IDs equal organization IDs. A known certifier profile has no matching organization, so a blind backfill followed by a non-null constraint would fail or fabricate ownership.
*   **Decision:** DM-03 C2 preserves independent profile primary keys and adds nullable, indexed `organization_id` foreign keys to producers and certifiers. Matching equal-ID profiles are backfilled only when the organization exists; unmapped profiles remain `NULL`. The schema permits future 1:N organization-to-profile ownership. JWTs retain only `orgId`. RLS uses the reserved future transaction-local GUC `app.current_org`, but C2 creates no policy and does not enable RLS.
*   **Consequences:** Tenant ownership is now expressible and FK-backed without changing current authorization behavior. Activation and development fixtures populate explicit ownership for new profiles. Equal-ID C0 predicates, orphan resolution, `NOT NULL`, RLS enforcement, predicate joins, and laboratory assignment remain separately approved C3 work.
*   **Verification:** Migration 0011 has deterministic exact/absent/incompatible state verification and disposable coverage for guarded backfill, orphan quarantine, FK rejection, index/constraint validation, adoption, idempotency, and normalized schema equality. F2 profile creation and the exact F1 compliance contract remain regression gates.
...
## D-018: Tenant Constraint Tightening and Orphan Quarantine

*   **Status:** IMPLEMENTED — PENDING ARCHITECT C3c REVIEW
*   **Context:** C3a/C3b established explicit ownership authorization and mandatory application writes, but producer ownership and investigation provenance remained nullable, investigation links were not unique, and the sole legacy certifier without an organization remained active despite having zero budget references. The immutable seedless baseline also requires a valid empty-database migration path.
*   **Decision:** Forward migration 0013 fails closed unless producer ownership and investigation links contain zero NULLs and investigation links contain zero duplicate groups. Non-empty legacy environments must contain exactly the approved zero-reference orphan `00000000-0000-0000-0000-000000000003`; the schema-only empty bootstrap may contain zero certifiers. The migration sets `producers.organization_id` and `investigations.unit_code_id` `NOT NULL`, replaces the investigation support index with a same-name unique btree index, and changes only the approved orphan from `ACTIVE` to the existing reversible `REVOKED` state. `certifiers.organization_id` remains nullable.
*   **Consequences:** Producer tenant roots and investigation provenance are database-enforced, and each unit code can have at most one investigation. The known orphan cannot be selected as an active signer and remains available for later attach/create/delete disposition without invented ownership or destructive mutation. Laboratory submitter/assignment columns remain nullable for legacy compatibility. No application predicate, JWT, RLS, capacity, signature, ledger-ordering, or transparency-service behavior changes.
*   **Verification:** `verify0013` classifies exact/absent/incompatible state including quarantine, while `verify0011`/`verify0012` recognize only the complete approved successor shape. Once empty bootstrap has recorded 0013, a later all-mapped certifier population with zero orphans remains exact; the same non-empty zero-orphan state is incompatible before execution. Disposable tests cover empty bootstrap, snapshot adoption, direct and runner idempotency, preflight rollback, NULL/UNIQUE rejection, orphan exclusion, and normalized forward/snapshot parity. A configured-database clone applies 0013 and ends migration-safe with the expected 0/0/0/1 preflight counts.

---

## D-019: Certifier Orphan Deletion and Mandatory Ownership

*   **Status:** IMPLEMENTED — PENDING ARCHITECT REVIEW
*   **Context:** Migration 0013 left the exact zero-budget certifier `00000000-0000-0000-0000-000000000003` quarantined as `REVOKED`, so `certifiers.organization_id` could not yet be mandatory. The operator separately approved deletion of that row.
*   **Decision:** Forward migration 0014 locks and deletes only the approved certifier ID when it remains `REVOKED` and has zero budget references. A live or referenced row is never deleted. After rejecting every remaining NULL certifier owner, the migration validates a temporary `NOT VALID` CHECK, sets `certifiers.organization_id NOT NULL`, and drops the redundant CHECK. Laboratory submitter and assignment columns remain nullable.
*   **Consequences:** Every surviving producer and certifier profile now has an FK-backed organization owner. Empty bootstrap and rerun states remain idempotent. No RLS, service code, authorization, JWT, capacity, signature, ledger, or transparency-service behavior changes.
*   **Verification:** `verify0014` is exact/absent/incompatible. The 0011–0013 verifiers accept the post-0014 shape only with the 0014 `migrations_log` record. Disposable PostgreSQL coverage proves empty bootstrap, exact guarded deletion, rejection of unexpected NULLs, rejection of active/referenced deletion targets, low-lock tightening, laboratory over-tightening rejection, direct rerun, ledger anchoring, and normalized forward/snapshot equality.

---

## D-020: Non-Owner Tenant Context Foundation

*   **Status:** IMPLEMENTED — PENDING ARCHITECT D1 REVIEW
*   **Context:** The six PostgreSQL-backed services connected as the schema owner and had no request-scoped database tenant context. RLS cannot be safely enabled until runtime queries use a dedicated non-owner and set tenant identity on the same transaction-bound pooled connection.
*   **Decision:** Forward migration 0015 creates `capmint_app` as a non-login, non-owner role without superuser, role-creation, database-creation, inheritance, replication, or RLS-bypass capability. It grants current and owner-default future application DML and sequence access while refusing any existing RLS enforcement. A single shared `withTenantTx` helper sets `app.current_organization_id` and `app.actor_is_system_admin` transaction-locally from the existing JWT claims. Authenticated actors without an organization fail before connection checkout unless they are an existing system administrator. Consumer resolver, public verification, and public transparency reads use an explicit public context.
*   **Consequences:** Service queries are ready for later defense-in-depth policies but D1 changes no result visibility: no policy exists and no table enables or forces RLS. The operator must separately provision a generated `capmint_app` login credential and rotate the six service `DATABASE_URL` secrets; migrations, bootstrap, and seeds remain owner operations. D3 must design policies for explicitly public database paths before public enforcement.
*   **Verification:** Migration 0015 has exact/absent/incompatible verification, guarded idempotency, complete privilege evidence, and explicit no-RLS checks. Real PostgreSQL lifecycle coverage proves both GUC values, fail-closed missing-org behavior, no cross-request pool leakage, and clearing after commit. The app-role tenant suite passes 14/14, the bootstrap/seed suite passes 8/8, and the unchanged compliance contract passes 88/0/0 while all six services connect as `capmint_app`.
