...
server.post('/api/v1/verify/lab-results', {
  ...
  const { lot_id, lab_name, test_type, result_summary, report_hash, report_reference } = request.body as any;
  ...
  try {
    ...
    await client.query(
      ...
      [lot_id, result_summary === 'PASSED' ? 'PASSED' : 'FAILED']
    );
...
const start = async () => {
  try {
    ...
    try {
      ...
      server.log.error(dbErr as any, 'Failed to initialize investigations table');
    } finally {
      client.release();
    }
