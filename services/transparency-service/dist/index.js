...
server.post('/api/v1/log', async (request, reply) => {
    const { entity_type, entity_id, event_type, payload } = request.body;
    ...
    try {
        ...
        const latestRes = await client.query('SELECT current_hash FROM log_entries ORDER BY created_at DESC, id DESC LIMIT 1 FOR UPDATE');
...
async function handleVerifyLog(request, reply) {
    try {
...
async function handleGetEntries(request, reply) {
...
async function handleAppendLog(request, reply) {
    const { entity_type, entity_id, event_type, payload } = request.body;
    if (!entity_type || !entity_id || !event_type || !payload) {
        return reply.status(400).send({
            success: false,
...
const start = async () => {
    try {
        ...
        `);
            ...
            server.log.error(seedErr, 'Seeding log_entries failed');
        }
        finally {
            client.release();
        }
