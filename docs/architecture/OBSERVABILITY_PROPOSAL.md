...
| **O3 — Metrics** ← **next (last slice)** | `prom-client` `/metrics` per service: HTTP histograms + domain-security counters (RLS `42501` denials, capacity rejections, `INVALID_SIGNATURE`, auth failures/rate-limits, ledger append rate). Error counter hangs off the O4 handler's insertion point. | HO-010 |
