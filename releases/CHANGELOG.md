...
- **Complete Capacity Signature Enforcement (High)**: Explicit lot creation now verifies the active budget's Ed25519 certifier authorization under the existing row lock before reserving capacity. Drawdown, lot reservation, mint, explicit registration, and quick registration all reject invalid signatures without database or ledger mutation.
- **Secure Environment Bootstrap (Critical)**: Removed auth/CPQ startup fixture DML and the unsafe standalone seed SQL. Empty production environments now create their first system administrator only through an explicit, advisory-locked, transactional command with an injected strong password and immutable audit event. Explicit development fixtures require a non-production allowlist, enable flag, matching supplied Ed25519 pair, and reject the compromised historical key.
...
- **State-Aware Migration Reconciliation**: Added explicit check, plan, apply, adopt, and empty-database bootstrap modes; truthful `LEGACY`/`EXECUTED`/`ADOPTED`/`BASELINE` provenance; immutable checksummed baseline bootstrap; advisory locking; forward reconciliation migration `0010`; and disposable PostgreSQL schema-drift verification. The development database later adopted exact 0007/0009 effects and executed 0010 through the reviewed runner.
- **Tenant-Scoped Compliance Baseline**: Reconciled the live HTTP suite with D-011, added exact owner/other/anonymous list and export checks, cross-tenant mutation and zero-state-change proofs, all-capacity-path signature regressions, explicit laboratory pending coverage, fatal exit propagation, and a database-name guard. CI now runs the exact 83 PASS / 5 PENDING / 0 FAIL contract on disposable PostgreSQL and isolated Redis state.
- **Bootstrap/Seed Separation**: Kept the baseline schema-only, defined no production reference fixtures, added explicit first-admin and development-seed commands, and moved the disposable compliance harness off implicit application startup seeding.
- **Explicit Profile Tenant Ownership Foundation**: Added nullable, indexed, FK-backed `organization_id` columns to producer and certifier profiles through forward migration `0011`. Matching equal-ID profiles are backfilled only when their organization exists; unmapped legacy profiles remain quarantined as `NULL`. Activation and development fixtures now populate the explicit owner. JWT claims, C0 predicates, RLS enforcement, `NOT NULL`, orphan resolution, and laboratory assignment remain unchanged for separately approved DM-03 C3.
- **Derived Tenant Enforcement (DM-03 C3a)**: Added forward migration `0012` with exact investigation-to-unit provenance, lot/budget producer consistency, and nullable laboratory relationship preparation. Producer and certifier authorization now resolves through `profile.organization_id = jwt.orgId` in scoped SQL locks/reads across budget, mint, lot, export, certification, revocation, and investigation routes. Independent profile IDs are supported without changing JWT claims. RLS, laboratory assignment behavior, `NOT NULL`/uniqueness tightening, and orphan resolution remain unimplemented behind later approval gates.
- **Laboratory Tenant Enforcement (DM-03 C3b)**: Added the certifier-scoped, idempotent lot laboratory-assignment API; restricted activated laboratories to assigned lot lists and writes; persisted authenticated submitter provenance for new/replaced reports; retained readable legacy `NULL` submitters; populated new investigation provenance at creation; allowlisted producer/certifier/system-admin integration lookups; and aligned frontend authenticated operations. The disposable compliance contract now passes 88/0/0 twice. No migration, RLS, nullability/uniqueness tightening, capacity/signature change, or transparency-service change is included.
- **Tenant Constraint Tightening (DM-03 C3c)**: Added fail-closed migration `0013`, making producer ownership and investigation unit provenance mandatory and enforcing one investigation per unit code. The exact zero-budget legacy certifier is reversibly quarantined as `REVOKED`; its organization link remains nullable pending separately approved disposition. Empty schema-only bootstrap remains supported. Laboratory tenant columns stay nullable, and no RLS, authorization-predicate, capacity, signature, ledger-ordering, or transparency-service behavior changed.
- **Certifier Ownership Enforcement (DM-03 follow-up)**: Added migration `0014`, which deletes only the operator-approved revoked certifier `00000000-0000-0000-0000-000000000003` after proving zero budget references, rejects all other NULL owners, and makes `certifiers.organization_id` mandatory through a validated low-lock CHECK sequence. Empty bootstrap and reruns remain idempotent; laboratory tenant columns stay nullable, and no RLS or service behavior changed.
- **Non-Owner RLS Foundation (DM-04 D1)**: Added migration `0015` with the non-owner, non-login, non-bypass `capmint_app` role and complete current/future application DML and sequence grants. All six PostgreSQL-backed services now use one transaction helper that sets request-local organization and system-admin GUCs on the exact pooled client running each query. Public routes opt into an explicit unscoped context, and the operator must separately provision the role credential and rotate service `DATABASE_URL` secrets. No RLS policy is created or enabled in D1.

### Known Gaps (carried forward, not yet fixed)
- Follow-ups: PostgreSQL RLS policy/enforcement phases DM-04 D2/D3, public-access policy design, JWT token revocation (denylist), the unexpiring internal ledger service token, and repository hygiene are tracked separately.
